...
numBarsToGoBack = 1000 # number of bars from the end of data
...
#        if (mp < 0 and maxPositionS < 3) : maxPositionS = mp
 ...
 #Long Entry Logic
        if (mp == 0 or mp == -1)  and myClose[i] >= myXavg:
            profit = 0
            price = myClose[i]
            if mp <= -1:
 ...
 # Long Exit - Time Based
        if mp >= 1 and myClose[i] < entryPrice[-1] and barsSinceEntry >= 10:
            price = myClose[i]
...
# Short Exit Loss
        ...
        if mp <= -1 and myHigh[i] >= entryPrice[-1] + stopAmt and barsSinceEntry > 1:
            price = max(myOpen[i],entryPrice[-1] + stopAmt)
            tradeName = "S-MMLoss"
            exitDate = myDate[i]
            numShares = curShares
            exitQuant.append(numShares)
...
# Short Exit Profit
 ...
 ###########  DO NOT CHANGE BELOW ################################################################
    ...
    if mp <= -1:
        ...
        profit,trades,curShares = exitPos(price,myDate[i],tradeName,numShares)
        listOfTrades.append(trades)
    systemMarket.setSysMarkInfo("XavgCross",myComName,listOfTrades,equity)
